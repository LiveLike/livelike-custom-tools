// Tabs Related
const switchTab = (e) => {
  if (e.target.className !== 'tab-item active') {
    document.querySelector('.tab-item.active').className = 'tab-item';
    document.querySelector('.tab-contents.active').className = 'tab-contents';

    e.target.className = 'tab-item active';
    document.querySelector('#' + e.target.id + '-contents').className =
      'tab-contents active';
  }
};

const tabOneButtons = document.querySelectorAll('.tab-nav.one > button');

tabOneButtons.forEach((btn, idx) => btn.addEventListener('click', switchTab));
...
const getLiveLikeData = () => {
  ...
  if (params.programid) {
    ...
    return Promise.resolve({
      publicChatId: '1b8792b2-eadc-4355-a323-72a028745e0b',
      influencerChatId: '39a245dd-2a83-42e8-83fb-442d7adad0f6',
      programId: '9baf1962-f7db-43b9-ae8e-b84f1bf31988',
      endpoint: 'https://cf-blast.livelikecdn.com/api/v1/',
      clientId: 'jV2LSongmdccPRL65M8GC6Z8lTievPsRsqsoSMiq',
      cheerMeterProgramId: '0ccaa346-8022-4a51-8089-99a78103ac8a',
    });
  }
};
